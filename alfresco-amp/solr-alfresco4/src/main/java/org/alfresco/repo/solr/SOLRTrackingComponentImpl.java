...
public class SOLRTrackingComponentImpl implements SOLRTrackingComponent
{
	...
	protected Map<QName, Serializable> getProperties(Long nodeId)
    {
        ...
        props.putAll(getAssocProperties(nodeId, props));
        return props;
    }

	protected Map<QName, Serializable> getAssocProperties(Long nodeId, Map<QName, Serializable> props){
    	Collection<Pair<Long, AssociationRef>> assoc = nodeDAO.getTargetNodeAssocs(nodeId, null);
    	Map<QName, Serializable> results = new HashMap<QName, Serializable>();
    	for (Pair<Long, AssociationRef> pair : assoc) {
    		if (pair.getSecond().getTypeQName().getLocalName().endsWith("follow")){
    			Map<QName, Serializable> properties= getProperties(nodeDAO.getNodePair(pair.getSecond().getTargetRef()).getFirst());
    			for (QName key : properties.keySet()) {
    				PropertyDefinition propDef = dictionaryService.getProperty(key);
    				if (propDef != null && !key.getNamespaceURI().startsWith("http://www.alfresco.org"))
    					addToPropertyMap(results, key, properties.get(key));
				}
    		}
		}
	...
	@SuppressWarnings("unchecked")
	private void addToPropertyMap(Map<QName, Serializable> properties, QName key, Serializable value){
		if (properties.get(key) != null){
			List<Serializable> values = (List<Serializable>) properties.get(key);
            if (value instanceof Collection<?> || value instanceof Map<?, ?>){
            	for (Serializable serializable : (List<Serializable>)value) {
