...
public class SolrCMISQueryServiceImpl implements CMISQueryService
{
    ...
    public CMISResultSet query(CMISQueryOptions options)
    {
        ...
        CMISResultSet cmis = new CMISResultSetImpl(wrapped, options, limitBy, nodeService, query, cmisDictionaryService, alfrescoDictionaryService, rs.getNumberFound()
        		
        		);
     
        
        List<String> facets = rs.getFieldFacets();
        
        cmis.setFieldFacets(facets);
        
        for(String facet : facets)
        	cmis.addFacet( facet, rs.getFieldFacet(facet) );
